"""Nosis cell evaluation — reference semantics for IR primitives.

Every IR PrimOp has its reference evaluation function defined here.
Constant folding, truth-table passes, and cone evaluation all call
into this module. The one exception is ``nosis.sim.FastSimulator``,
which flattens the same semantics into a dispatch table for speed —
any semantic change here must be mirrored there.
