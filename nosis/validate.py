...
def _lines_match(rtl_line: str, synth_line: str) -> bool:
    """Compare an RTL and post-synth output line for equivalence.

    RTL is the golden reference. Where an RTL bit is ``x`` (a don't-care —
    e.g. an uninitialized register on an inactive path), any synthesized
    value matches; this is standard RTL-vs-gate comparison and avoids
    flagging legitimate X-resolution (an FF powering up to 0) as a bug.
    Where the RTL bit is defined (0/1), the synth bit must equal it, so a
    synthesized value that drops or corrupts a defined bit still fails.
    """
    if rtl_line == synth_line:
        return True
    rtl_tokens = rtl_line.split()
    synth_tokens = synth_line.split()
    if len(rtl_tokens) != len(synth_tokens):
        return False
    for rt, st in zip(rtl_tokens, synth_tokens):
        if rt == st:
            continue
        if "=" not in rt or "=" not in st:
            return False
        rn, rv = rt.split("=", 1)
        sn, sv = st.split("=", 1)
        if rn != sn:
            return False
        # A fully-don't-care RTL value matches any synthesized value, even if
        # the two sims format the width differently (e.g. "xxxxxxxx" vs "0").
        if set(rv) <= {"x"}:
            continue
        if len(rv) != len(sv):
            return False
        for rc, sc in zip(rv, sv):
            if rc == "x":
                continue  # RTL don't-care — any synthesized value is fine
            if rc != sc:
                return False
    return True
...
def validate_design(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="nosis_val_") as tmp:
        ...
        # generate_postsynth_verilog embeds the behavioral cell models,
        # so the netlist file is self-contained for iverilog.
        from nosis.postsynth import generate_postsynth_verilog
        postsynth_source = generate_postsynth_verilog(netlist)

        # Apply the SAME stimulus to both simulations — the comparison is
        # meaningless otherwise.
        if vectors:
            synth_tb = generate_testbench_from_vectors(
                f"{mod.name}_postsynth", ports, vectors,
        ...
        synth_ok, synth_err, synth_lines = _run_iverilog_sim(
            iverilog, vvp, [str(postsynth_path)],
            synth_tb, work, output_file="synth_output.txt", label="synth",
        )

        if not synth_ok:
            # A post-synthesis sim that fails to run is a validation
            # failure — reporting RTL-vs-RTL here would be a false PASS.
            return ValidationResult(
                design=design_name, passed=False, cycles=len(rtl_lines),
                rtl_sim_ok=True, synth_sim_ok=False,
                error=f"post-synth sim: {synth_err}",
            )

        mismatches: list[dict[str, Any]] = []
        for i, (rtl_line, synth_line) in enumerate(zip(rtl_lines, synth_lines)):
            if not _lines_match(rtl_line, synth_line):
                mismatches.append({
                    "cycle": i,
                    "rtl": rtl_line,
                    "synth": synth_line,
