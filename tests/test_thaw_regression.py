...
def test_per_write_we_numbering():
    ...
    for cell in mod.cells.values():
        if cell.op == PrimOp.MEMORY and "resp" in cell.params.get("mem_name", ""):
            ...
            assert "WE1" in cell.inputs, "WE1 missing — first port WE collides with global OR"
            assert "WE" in cell.inputs, "Global OR WE missing"
...
def test_memory_mapping_types():
    ...
    assert found["SVC.resp"] != "DP16KD", "resp must NOT use DP16KD (has 108 simultaneous writes)"
