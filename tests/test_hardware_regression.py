...
def test_iterative_merge_reduces_more_than_single_pass():
    ...
    from nosis.slicepack import merge_lut_chains
    os.environ["NOSIS_BRAM_THRESHOLD"] = "0"
    result = parse_files([_CPU_SV], top="rime_v")
    design = lower_to_ir(result, top="rime_v")
    mod = design.top_module()
    ...
    netlist = map_to_ecp5(design)
    # Single pass
    round1 = merge_lut_chains(netlist)
    round2 = merge_lut_chains(netlist)
...
# ---------------------------------------------------------------------------
# No LUT4 input may read an undriven bit. A constant-output LUT must have
# its consumers tied to "0"/"1" (constant propagation), never left dangling.
# ---------------------------------------------------------------------------
...
def test_no_lut4_with_undriven_signal_inputs():
    ...
    assert len(bad_luts) == 0, (
        f"{len(bad_luts)} LUT4 cells have undriven signal inputs — a "
        f"constant-output LUT was deleted without propagating its value "
        f"to consumers, or the mapper wired a pin to an undriven bit."
    )
