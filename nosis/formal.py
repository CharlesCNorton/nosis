...
Assertion checking, reachability analysis, and sequential equivalence
via simulation. For small combinational designs the assertion check is
exhaustive over all input combinations (provably complete); everything
else is simulation-based with a fixed bound and clearly labeled as such
in the result's ``method`` field.
"""
...
def check_assertion_bmc_sat(
    ...
    *,
    ...
    """Exhaustive bounded model checking for combinational designs.

    For combinational designs with at most 20 input bits, evaluates the
    property ``output_net == expected_value`` for every input combination.
    A pass is a proof over the full input space.

    For sequential designs or wider inputs, falls back to the random
    simulation BMC of :func:`check_assertion_bmc`.
    """
