...
def test_chain_merge_absorbs_buffer():
    """A 1-input identity LUT feeding a single consumer merges away."""
    nl = ECP5Netlist(top="test")
    ...
    # cons now computes AND(2, 3) directly on the buffer's source signal
    assert set(b for p in "ABCD" for b in cons.ports[p] if isinstance(b, int)) == {2, 3}
...
def test_pack_slices_combined():
    ...
    zs = []
    for i in range(4):
        cell = _make_lut4(nl, f"lut{i}", format(0x8888, "016b"), a, b)
        zs.append(cell.ports["Z"][0])
    # Keep all four outputs live so dedup (not dead-LUT removal) collapses them
    nl.ports["out"] = {"direction": "output", "bits": zs}
