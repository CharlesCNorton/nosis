...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.ecppack and args.output:
        ...
        if not nextpnr_cmd:
            ...
            # OSS CAD Suite tools need their bin/ and lib/ dirs on PATH
            # for shared library resolution (matches pnr_feedback.run_nextpnr).
            tool_env = dict(_os.environ)
            tool_dir = str(Path(nextpnr_cmd).parent)
            tool_env["PATH"] = tool_dir + _os.pathsep + tool_env.get("PATH", "")
            lib_dir = str(Path(nextpnr_cmd).parent.parent / "lib")
            if Path(lib_dir).is_dir():
                tool_env["PATH"] = lib_dir + _os.pathsep + tool_env["PATH"]
            try:
                ...
                pnr_run = subprocess.run(nextpnr_args, check=True, capture_output=True, text=True, env=tool_env)
                if args.verbose:
                ...
                # Parse the captured nextpnr log for timing feedback
                if args.verbose or args.stats:
                    from nosis.pnr_feedback import parse_nextpnr_log
                    pnr = parse_nextpnr_log(pnr_run.stderr)
    ...
    # Shared analyses for the report modes — computed once
    if args.stats or args.verbose or args.benchmark or args.json_stats:
    ...
    if (args.stats or args.verbose) and not args.json_stats:
    ...
    # --- --benchmark mode ---
    if args.benchmark and not args.json_stats:
        bench = {
