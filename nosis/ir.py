...
class PrimOp(Enum):
    ...
    ROM = auto()        # constant lookup table: A (selector) -> Y; params: values, sel_width
...
def emit_verilog(mod: Module) -> str:
    ...
    for cell in mod.cells.values():
        ...
        if cell.op == PrimOp.CONST:
            ...
            a = _verilog_id(cell.inputs["A"].name) if "A" in cell.inputs else "0"
            in_w = cell.inputs["A"].width if "A" in cell.inputs else 1
            pad = max(out_nets[0].width - in_w, 1)
            lines.append(f"  assign {out} = {{{{{pad}{{1'b0}}}}, {a}}};")
