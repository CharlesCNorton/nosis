...
Nosis is a pure-Python FPGA synthesis tool. It reads SystemVerilog and Verilog, converts behavioral hardware descriptions into a technology-independent intermediate representation, optimizes the IR through an iterative pass pipeline (including HoTT-inspired equivalence merging and SAT-based constant proof), maps onto ECP5 FPGA primitives, and emits a JSON netlist consumable by nextpnr-ecp5 for place and route.
...
Source files (.sv, .v)
   ...
   [frontend]      nosis/frontend.py -- walk AST, emit IR cells and nets
       ...
       v
   [optimization]  nosis/passes/ -- 11 passes x up to 6 rounds + post-passes
       |            constant fold, identity, boolean, tech-aware, CSE,
       |            HIT merge, don't-care, MUX merge, DCE,
       |            reqmerge (FNV-1a hashing), SAT const/equiv, BDD,
       |            retiming (forward), fanout duplication
       v
...
Every PrimOp has its reference evaluation function in `eval.py`. Constant folding, equivalence checking, and truth table computation all use the same code path; the flat-array `FastSimulator` mirrors these semantics for speed. Any inconsistency between optimization and verification would require a bug in the shared semantics.
...
- Cofiber/zero object for dead LUT elimination
