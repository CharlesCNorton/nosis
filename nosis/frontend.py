...
from nosis.blackbox import ECP5_BLACKBOX_NAMES as _VENDOR_PRIMITIVES  # noqa: E402

import re as _re  # noqa: E402

# The signed marker is only ever `s`; including `h` in that class let the
# optional group swallow the base of a hex literal whose first digit looks
# like a base char (`64'hdeadbeef` -> base `d`; `8'hb0` -> binary `0`,
# silently wrong).
_VERILOG_LITERAL_RE = _re.compile(
    r"^(\d+)'([sS]?)([bBoOdDhH])([0-9a-fA-FxXzZ_]+)$"
)
...
def parse_files(
    ...
    *,
    ...
    cmd_parts: list[str] = [
        ...
        "--timescale=1ns/1ps",
        # Tolerate signals used before their declaration. Several
        # silicon-proven RIME modules reference a reg/wire in a port list
        # or expression above its own declaration line; without this, slang
        # leaves the connection or the entire procedural block Invalid, and
        # the logic (whole always_ff blocks) silently vanishes.
...
class _Lowerer:
    ...
    def __init__(self, module: Module) -> None:
        ...
        # Active function-inline scopes: (unique_prefix, {formal_name: actual_net}).
        # While set, net-name resolution maps formals to their call arguments and
        # gives every other function-local name a per-call-site prefix so repeated
        # calls to the same function do not collide.
        self._fn_scopes: list[tuple[str, dict[str, Net]]] = []
        self._fn_call_ctr = 0

    def _fn_resolve(self, name: str, width: int) -> Net | None:
        """Resolve a name inside the active function-inline scope, or None.

        Formals map to their bound actual nets; everything else is created
        under the call-site prefix so it is unique per call.
        """
        if not self._fn_scopes:
            return None
        prefix, fmap = self._fn_scopes[-1]
        if name in fmap:
            return fmap[name]
        if width <= 0:
            width = 1
        qn = f"{prefix}{name}"
        ex = self.mod.nets.get(qn)
        if ex is not None:
            if ex.width == 1 and width > 1 and ex.driver is None:
                ex.width = width
            return ex
        return self.mod.add_net(qn, width)
    ...
    def _get_or_create_net(self, name: str, width: int) -> Net:
        fn = self._fn_resolve(name, width)
        if fn is not None:
            return fn
        if width <= 0:
    ...
    def lower_expr(self, expr: Any) -> Net:
        ...
        if kind == "ExpressionKind.IntegerLiteral":
            ...
            # User-defined function: inline the body per call site.
            sub = getattr(expr, "subroutine", None)
            if sub and hasattr(sub, "body") and sub.body is not None:
                ret = self._inline_function(sub, expr)
                if ret is not None:
                    return ret
    ...
    def _const_val(self, net: Net, depth: int = 0) -> int | None:
        """Evaluate a net to an int if its cone is all constants, else None.

        Lets unrolled loop indices (e.g. ``i*8`` with ``i`` swapped to a
        CONST) collapse to a static slice offset without waiting for the
        constant-folding pass.
        """
        if net is None or depth > 24:
            return None
        d = net.driver
        if d is None:
            return None
        if d.op == PrimOp.CONST:
            return int(d.params.get("value", 0))
        from nosis.eval import eval_const_op
        inputs: dict[str, int] = {}
        for pn, inet in d.inputs.items():
            v = self._const_val(inet, depth + 1)
            if v is None:
                return None
            inputs[pn] = v
        params = dict(d.params)
        if d.op == PrimOp.CONCAT:
            for pn, inet in d.inputs.items():
                params[f"{pn}_width"] = inet.width
        try:
            return eval_const_op(d.op, inputs, params, net.width)
        except Exception:
            return None

    def _slice_read(self, src: Net, offset: int, width: int) -> Net:
        out = self._fresh_net("slice", width)
        cell = self._fresh_cell("slice", PrimOp.SLICE, offset=offset, width=width)
        self.mod.connect(cell, "A", src)
        self.mod.connect(cell, "Y", out, direction="output")
        return out

    def _dyn_partsel_read(self, src: Net, idx_net: Net, width: int, is_down: bool) -> Net:
        """Read `src[base +/- : width]` with a runtime base as a per-bit select.

        result[k] = src[lo + k], lo = base (up) or base-(width-1) (down).
        Each result bit is a dynamic select over src positions; no wide
        shift (nosis's barrel shifter caps at 6 stages).
        """
        W = src.width
        out_bits: list[Net] = []
        for k in range(width):
            # source position = idx + k (up) or idx + k - (width-1) (down)
            add_k = k if not is_down else (k - (width - 1))
            cur = self._fresh_net(f"dpr_z{k}", 1)
            zc = self._fresh_cell(f"dpr_z{k}", PrimOp.CONST, value=0, width=1)
            self.mod.connect(zc, "Y", cur, direction="output")
            for j in range(W):
                # src bit j is selected when idx + add_k == j  ->  idx == j - add_k
                target = j - add_k
                if target < 0 or target >= (1 << idx_net.width):
                    continue
                sbit = self._fresh_net(f"dpr_s{k}_{j}", 1)
                sc = self._fresh_cell(f"dpr_s{k}_{j}", PrimOp.SLICE, offset=j, width=1)
                self.mod.connect(sc, "A", src)
                self.mod.connect(sc, "Y", sbit, direction="output")
                cnet = self._fresh_net(f"dpr_c{k}_{j}", idx_net.width)
                cc = self._fresh_cell(f"dpr_c{k}_{j}", PrimOp.CONST, value=target, width=idx_net.width)
                self.mod.connect(cc, "Y", cnet, direction="output")
                eq = self._fresh_net(f"dpr_eq{k}_{j}", 1)
                eqc = self._fresh_cell(f"dpr_eq{k}_{j}", PrimOp.EQ)
                self.mod.connect(eqc, "A", idx_net)
                self.mod.connect(eqc, "B", cnet)
                self.mod.connect(eqc, "Y", eq, direction="output")
                mo = self._fresh_net(f"dpr_m{k}_{j}", 1)
                mx = self._fresh_cell(f"dpr_m{k}_{j}", PrimOp.MUX)
                self.mod.connect(mx, "S", eq)
                self.mod.connect(mx, "A", cur)
                self.mod.connect(mx, "B", sbit)
                self.mod.connect(mx, "Y", mo, direction="output")
                cur = mo
            out_bits.append(cur)
        out = self._fresh_net("dpr_cat", width)
        cat = self._fresh_cell("dpr_cat", PrimOp.CONCAT, count=width)
        for i, b in enumerate(out_bits):
            self.mod.connect(cat, f"I{i}", b)
        self.mod.connect(cat, "Y", out, direction="output")
        return out

    def _lower_range_select(self, expr: Any) -> Net:
        w = self._bit_width(expr)
        src = self.lower_expr(expr.value)
        sk = str(getattr(expr, "selectionKind", "Simple"))

        if "Indexed" in sk:
            is_down = "IndexedDown" in sk
            wc = getattr(getattr(expr, "right", None), "constant", None)
            sel_w = _svint_to_int(wc) if wc is not None else w
            base_node = expr.left
            base_c = getattr(base_node, "constant", None)
            if base_c is not None:
                base = _svint_to_int(base_c)
                lo = base if not is_down else base - (sel_w - 1)
                return self._slice_read(src, max(0, lo), sel_w)
            idx_net = self.lower_expr(base_node)
            cv = self._const_val(idx_net)
            if cv is not None:
                lo = cv if not is_down else cv - (sel_w - 1)
                return self._slice_read(src, max(0, lo), sel_w)
            wa = self._word_aligned_partsel(base_node, sel_w, is_down, src.width)
            if wa is not None:
                var_net, sel_values = wa
                return self._word_partsel_read(src, var_net, sel_values, sel_w)
            return self._dyn_partsel_read(src, idx_net, sel_w, is_down)

        # Simple `[hi:lo]` — offset is the (constant) low index.
        offset = 0
        right_val = getattr(expr, "right", None)
        rc = getattr(right_val, "constant", None) if right_val is not None else None
        if rc is not None:
            offset = _svint_to_int(rc)
        elif right_val is not None and getattr(right_val, "value", None) is not None:
            offset = _svint_to_int(right_val.value)
        return self._slice_read(src, offset, w)
        ...
        if kind == "StatementKind.ExpressionStatement":
            ...
            if str(expr.kind) == "ExpressionKind.Assignment":
                if not expr.isNonBlocking or allow_nb:
                    ...
                    elif self._const_partsel_write(expr.left) is not None:
                        # reg[hi:lo] <= x — read-modify-write the whole reg.
                        base_node, hi, lo = self._const_partsel_write(expr.left)
                        base_net = self.lower_expr(base_node)
                        rhs = self.lower_expr(expr.right)
                        hold = _running.get(base_net.name, base_net) if _running else base_net
                        results[base_net.name] = self._expand_partsel_write(base_net, hi, lo, rhs, hold)
                    elif self._dyn_partsel_write(expr.left) is not None:
                        # reg[base +/-: w] <= x — variable-offset part-select.
                        base_node, idx_node, psw, is_down = self._dyn_partsel_write(expr.left)
                        base_net = self.lower_expr(base_node)
                        idx_net = self.lower_expr(idx_node)
                        rhs = self.lower_expr(expr.right)
                        hold = _running.get(base_net.name, base_net) if _running else base_net
                        cv = self._const_val(idx_net)
                        wa = None if cv is not None else self._word_aligned_partsel(
                            idx_node, psw, is_down, base_net.width)
                        if cv is not None:
                            # Constant offset (e.g. an unrolled loop index) —
                            # collapse to a static read-modify-write instead of
                            # the per-bit MUX fabric.
                            lo = cv if not is_down else cv - (psw - 1)
                            results[base_net.name] = self._expand_partsel_write(
                                base_net, lo + psw - 1, max(0, lo), rhs, hold)
                        elif wa is not None:
                            # Word-aligned lane write — one MUX per word.
                            var_net, sel_values = wa
                            results[base_net.name] = self._word_partsel_write(
                                base_net, var_net, sel_values, psw, rhs, hold)
                        else:
                            results[base_net.name] = self._expand_dyn_partsel_write(
                                base_net, idx_net, psw, is_down, rhs, hold)
                    else:
                        lhs = self.lower_expr(expr.left)
                        rhs = self.lower_expr(expr.right)
            ...
            for inner_item in stmt.items:
                ...
                for ieq in case_eqs:
                    for tn, rv in inner_map.items():
                        ...
                        self.mod.connect(mx, "Y", mo, direction="output")
                        inner_running[tn] = mo
                    # Targets the DEFAULT arm assigns but this explicit arm
                    # does not: the chain tail is the default's value, so
                    # without a node this arm would take the default instead
                    # of holding. Insert a hold node.
                    for tn in inner_default:
                        if tn in inner_map:
                            continue
                        tnet = self.mod.nets.get(tn)
                        if tnet is None:
                            continue
                        hold_val = (_running or {}).get(tn)
                        if hold_val is None:
                            if allow_nb:
                                hold_val = tnet
                            else:
                        ...
                        self.mod.connect(mx, "S", ieq)
                        self.mod.connect(mx, "A", prev)
                        self.mod.connect(mx, "B", hold_val)
                        self.mod.connect(mx, "Y", mo, direction="output")
                        inner_running[tn] = mo
            ...
            if body is not None:
                ...
                elif lvar_name is None:
                    # Loop variable declared outside the for-statement
                    # (`integer i; ... for (i = 0; ...)`), so slang leaves
                    # loopVars empty. Recover it from the init assignment.
                    for _ie in list(getattr(stmt, "initializers", []) or []):
                        if str(getattr(_ie, "kind", "")) != "ExpressionKind.Assignment":
                            continue
                        _lsym = getattr(getattr(_ie, "left", None), "symbol", None)
                        if _lsym is None:
                            continue
                        lvar_name = _lsym.name
                        _ic = getattr(getattr(_ie, "right", None), "constant", None)
                        if _ic is not None:
                            lvar_init = _svint_to_int(_ic)
                        break
                ...
                if iter_range is not None and 0 < n_iters <= 64:
                    ...
                    # the constant value.
                    lvar_w = 32
                    self._get_or_create_net(lvar_name, lvar_w)
    ...
    def _const_partsel_write(self, lhs_expr: Any) -> tuple[Any, int, int] | None:
        """Detect `reg[hi:lo] <= x` or `reg[i] <= x` with constant indices.

        Returns ``(base_value_node, hi, lo)`` or ``None``. Excludes memory /
        unpacked-array element targets (handled elsewhere) — only plain
        packed registers reach the read-modify-write expander. A constant
        single-bit select `reg[i]` is treated as the 1-bit slice `[i:i]`.
        """
        if str(lhs_expr.kind) == "ExpressionKind.ElementSelect":
            base = lhs_expr.value
            if str(base.kind) == "ExpressionKind.ElementSelect":
                return None  # array element — handled by memory/array write paths
            sel = getattr(lhs_expr, "selector", None)
            sc = getattr(sel, "constant", None) if sel is not None else None
            if sc is None:
                return None
            # Only plain packed scalars (a variable/logic bitvector), not arrays
            vsym = getattr(base, "symbol", None)
            vtype = getattr(vsym, "type", None) if vsym is not None else None
            if vtype is not None and getattr(vtype, "isUnpackedArray", False):
                return None
            i = _svint_to_int(sc)
            return base, i, i
        ...
        if hi < lo:
            hi, lo = lo, hi
        base = lhs_expr.value
        # The base must be a plain packed value, not an element of an array
        # that already has dedicated write handling.
        if str(base.kind) == "ExpressionKind.ElementSelect":
            return None
        return base, hi, lo

    def _expand_partsel_write(self, base_net: Net, hi: int, lo: int, rhs: Net, hold: Net) -> Net:
        """Build the full register value for `reg[hi:lo] <= rhs` (read-modify-write).

        new_reg = { hold[W-1:hi+1], rhs[hi-lo:0], hold[lo-1:0] }, assembled
        LSB-first as a CONCAT.
        """
        width = base_net.width
        sel_w = hi - lo + 1
        pieces: list[Net] = []
        if lo > 0:
            lo_net = self._fresh_net("psw_lo", lo)
            lo_cell = self._fresh_cell("psw_lo", PrimOp.SLICE, offset=0, width=lo)
            self.mod.connect(lo_cell, "A", hold)
            self.mod.connect(lo_cell, "Y", lo_net, direction="output")
            pieces.append(lo_net)
        if rhs.width != sel_w:
            mid_net = self._fresh_net("psw_mid", sel_w)
            mid_cell = self._fresh_cell("psw_mid", PrimOp.SLICE, offset=0, width=sel_w)
            self.mod.connect(mid_cell, "A", rhs)
            self.mod.connect(mid_cell, "Y", mid_net, direction="output")
            pieces.append(mid_net)
        else:
            pieces.append(rhs)
        if hi + 1 < width:
            hi_w = width - hi - 1
            hi_net = self._fresh_net("psw_hi", hi_w)
            hi_cell = self._fresh_cell("psw_hi", PrimOp.SLICE, offset=hi + 1, width=hi_w)
            self.mod.connect(hi_cell, "A", hold)
            self.mod.connect(hi_cell, "Y", hi_net, direction="output")
            pieces.append(hi_net)
        if len(pieces) == 1:
            return pieces[0]
        out = self._fresh_net("psw_cat", width)
        cat = self._fresh_cell("psw_cat", PrimOp.CONCAT, count=len(pieces))
        for i, p in enumerate(pieces):
            self.mod.connect(cat, f"I{i}", p)
            cat.params[f"I{i}_width"] = p.width
        self.mod.connect(cat, "Y", out, direction="output")
        return out

    def _dyn_partsel_write(self, lhs_expr: Any) -> tuple[Any, Any, int, bool] | None:
        """Detect `reg[base +: w]` / `reg[base -: w]` with a variable base.

        Returns ``(base_value_node, index_node, width, is_down)`` or ``None``.
        The width must be a compile-time constant; the base index is a
        runtime expression.
        """
        if str(lhs_expr.kind) != "ExpressionKind.RangeSelect":
            return None
        sk = str(getattr(lhs_expr, "selectionKind", ""))
        if "Indexed" not in sk:
            return None
        width_node = getattr(lhs_expr, "right", None)
        idx_node = getattr(lhs_expr, "left", None)
        if width_node is None or idx_node is None:
            return None
        wc = getattr(width_node, "constant", None)
        if wc is None:
            return None
        width = _svint_to_int(wc)
        if width <= 0:
            return None
        return lhs_expr.value, idx_node, width, "IndexedDown" in sk

    def _expand_dyn_partsel_write(self, base_net: Net, idx_net: Net, width: int,
                                  is_down: bool, rhs: Net, hold: Net) -> Net:
        """Expand `reg[base +/- : width] <= rhs` into a per-bit MUX tree.

        For IndexedDown `[base -: w]`, register bit ``j`` receives ``rhs[k]``
        when ``base == j - k + (w-1)``; for IndexedUp `[base +: w]`, when
        ``base == j - k``. Every other bit holds. No wide shifts (nosis's
        barrel shifter caps at 6 stages); the shared ``base == const`` EQ
        cells are collapsed by CSE downstream.
        """
        W = base_net.width
        # Pre-slice rhs bits (rhs is `width` wide)
        rhs_bits: list[Net] = []
        for k in range(width):
            rb = self._fresh_net(f"dps_r{k}", 1)
            rc = self._fresh_cell(f"dps_r{k}", PrimOp.SLICE, offset=k, width=1)
            self.mod.connect(rc, "A", rhs)
            self.mod.connect(rc, "Y", rb, direction="output")
            rhs_bits.append(rb)
        out_bits: list[Net] = []
        for j in range(W):
            cur = self._fresh_net(f"dps_h{j}", 1)
            hc = self._fresh_cell(f"dps_h{j}", PrimOp.SLICE, offset=j, width=1)
            self.mod.connect(hc, "A", hold)
            self.mod.connect(hc, "Y", cur, direction="output")
            for k in range(width):
                target = (j - k + (width - 1)) if is_down else (j - k)
                if target < 0 or target >= (1 << idx_net.width):
                    continue
                eq = self._fresh_net(f"dps_eq{j}_{k}", 1)
                eqc = self._fresh_cell(f"dps_eq{j}_{k}", PrimOp.EQ)
                cnet = self._fresh_net(f"dps_c{j}_{k}", idx_net.width)
                cc = self._fresh_cell(f"dps_c{j}_{k}", PrimOp.CONST, value=target, width=idx_net.width)
                self.mod.connect(cc, "Y", cnet, direction="output")
                self.mod.connect(eqc, "A", idx_net)
                self.mod.connect(eqc, "B", cnet)
                self.mod.connect(eqc, "Y", eq, direction="output")
                mo = self._fresh_net(f"dps_m{j}_{k}", 1)
                mx = self._fresh_cell(f"dps_m{j}_{k}", PrimOp.MUX)
                self.mod.connect(mx, "S", eq)
                self.mod.connect(mx, "A", cur)
                self.mod.connect(mx, "B", rhs_bits[k])
                self.mod.connect(mx, "Y", mo, direction="output")
                cur = mo
            out_bits.append(cur)
        out = self._fresh_net("dps_cat", W)
        cat = self._fresh_cell("dps_cat", PrimOp.CONCAT, count=W)
        for i, b in enumerate(out_bits):
            self.mod.connect(cat, f"I{i}", b)
        self.mod.connect(cat, "Y", out, direction="output")
        return out

    def _affine_of(self, node: Any) -> tuple[int, Any, int] | None:
        """Express an index expression as ``coeff*var + const``.

        Returns ``(coeff, var_node, const)`` with ``var_node`` the single
        variable (or ``None`` for a pure constant), else ``None`` when the
        expression is not affine in one variable.
        """
        k = str(node.kind)
        if k == "ExpressionKind.Conversion":
            return self._affine_of(node.operand)
        c = getattr(node, "constant", None)
        if c is not None:
            return (0, None, _svint_to_int(c))
        if k == "ExpressionKind.NamedValue":
            return (1, node, 0)
        if k == "ExpressionKind.BinaryOp":
            op = str(getattr(node, "op", ""))
            la = self._affine_of(node.left)
            ra = self._affine_of(node.right)
            ...
            if "Add" in op:
                if lv is not None and rv is not None:
                    return None
                return (lc + rc, lv if lv is not None else rv, lk + rk)
            if "Subtract" in op:
                if lv is not None and rv is not None:
                    return None
                if rv is not None:
            ...
            if "Multiply" in op:
                if lv is None:
                    return (lk * rc, rv, lk * rk)
                if rv is None:
                    return (rk * lc, lv, rk * lk)
                return None
        return None

    def _word_aligned_partsel(self, idx_node: Any, width: int, is_down: bool,
                              total_width: int) -> tuple[Net, list[int]] | None:
        """Detect a word-aligned dynamic part-select and return its selectors.

        For `reg[base +/- : width]` where `base` is affine in one variable
        with coefficient ``±width`` and the windows land on width-aligned
        boundaries, returns ``(var_net, sel_values)`` where ``sel_values[m]``
        is the variable value that selects word ``m``. Otherwise ``None`` —
        the caller falls back to the general per-bit expansion.
        """
        if width <= 0 or total_width % width != 0:
            return None
        aff = self._affine_of(idx_node)
        if aff is None:
            return None
        coeff, var_node, const = aff
        if var_node is None or abs(coeff) != width:
            return None
        num_words = total_width // width
        sel_values: list[int] = []
        for m in range(num_words):
            target = m * width + (width - 1) if is_down else m * width
            num = target - const
            if coeff == 0 or num % coeff != 0:
                return None
            sel_values.append(num // coeff)
        return self.lower_expr(var_node), sel_values

    def _word_partsel_write(self, base_net: Net, var_net: Net, sel_values: list[int],
                            width: int, rhs: Net, hold: Net) -> Net:
        """Word-granular `reg[...] <= x`: mux each aligned word by the index."""
        W = base_net.width
        num_words = W // width
        words: list[Net] = []
        for m in range(num_words):
            hw = self._slice_read(hold, m * width, width)
            sel = sel_values[m]
            if sel < 0 or sel >= (1 << var_net.width):
                words.append(hw)
                continue
            cnet = self._fresh_net(f"wps_c{m}", var_net.width)
            cc = self._fresh_cell(f"wps_c{m}", PrimOp.CONST, value=sel, width=var_net.width)
            self.mod.connect(cc, "Y", cnet, direction="output")
            eq = self._fresh_net(f"wps_eq{m}", 1)
            eqc = self._fresh_cell(f"wps_eq{m}", PrimOp.EQ)
            self.mod.connect(eqc, "A", var_net)
            self.mod.connect(eqc, "B", cnet)
            self.mod.connect(eqc, "Y", eq, direction="output")
            mo = self._fresh_net(f"wps_m{m}", width)
            mx = self._fresh_cell(f"wps_m{m}", PrimOp.MUX)
            self.mod.connect(mx, "S", eq)
            self.mod.connect(mx, "A", hw)
            self.mod.connect(mx, "B", rhs)
            self.mod.connect(mx, "Y", mo, direction="output")
            words.append(mo)
        out = self._fresh_net("wps_cat", W)
        cat = self._fresh_cell("wps_cat", PrimOp.CONCAT, count=num_words)
        for i, wnet in enumerate(words):
            self.mod.connect(cat, f"I{i}", wnet)
        self.mod.connect(cat, "Y", out, direction="output")
        return out

    def _word_partsel_read(self, src: Net, var_net: Net, sel_values: list[int],
                           width: int) -> Net:
        """Word-granular `src[...]` read: mux the aligned source word by index."""
        cur = self._fresh_net("wpr_z", width)
        zc = self._fresh_cell("wpr_z", PrimOp.CONST, value=0, width=width)
        self.mod.connect(zc, "Y", cur, direction="output")
        for m, sel in enumerate(sel_values):
            if sel < 0 or sel >= (1 << var_net.width):
                continue
            sw = self._slice_read(src, m * width, width)
            cnet = self._fresh_net(f"wpr_c{m}", var_net.width)
            cc = self._fresh_cell(f"wpr_c{m}", PrimOp.CONST, value=sel, width=var_net.width)
            self.mod.connect(cc, "Y", cnet, direction="output")
            eq = self._fresh_net(f"wpr_eq{m}", 1)
            eqc = self._fresh_cell(f"wpr_eq{m}", PrimOp.EQ)
            self.mod.connect(eqc, "A", var_net)
            self.mod.connect(eqc, "B", cnet)
            self.mod.connect(eqc, "Y", eq, direction="output")
            mo = self._fresh_net(f"wpr_m{m}", width)
            mx = self._fresh_cell(f"wpr_m{m}", PrimOp.MUX)
            self.mod.connect(mx, "S", eq)
            self.mod.connect(mx, "A", cur)
            self.mod.connect(mx, "B", sw)
            self.mod.connect(mx, "Y", mo, direction="output")
            cur = mo
        return cur

    def _inline_function(self, sub: Any, call_expr: Any) -> Net | None:
        """Inline a user-defined function call with a fresh per-call scope.

        Formals bind to the lowered call arguments; the return variable and
        every function-local net get a unique per-call prefix so repeated
        calls to the same function do not collide. The body is collected
        through the blocking-assignment engine (which already handles
        if/case MUX trees and bit/part-select writes), and the threaded
        return-variable value net is returned.
        """
        args_list = list(getattr(call_expr, "arguments", []))
        formals = list(getattr(sub, "arguments", []))
        fmap: dict[str, Net] = {}
        for formal, actual in zip(formals, args_list):
            fmap[formal.name] = self.lower_expr(actual)  # in the caller's scope
        self._fn_call_ctr += 1
        ret_name = getattr(sub, "name", "f")
        prefix = f"$fn{self._fn_call_ctr}_{ret_name}."
        self._fn_scopes.append((prefix, fmap))
        try:
            # Seed the return variable at 0 so bit/part-select writes that
            # read-modify-write it (e.g. `crc[0] = ...`) start from a defined
            # value instead of an undriven net.
            ret_net = self._fn_resolve(ret_name, self._bit_width(call_expr))
            if ret_net is not None and ret_net.driver is None:
                zc = self._fresh_cell("fn_ret_seed", PrimOp.CONST,
                                      value=0, width=max(1, ret_net.width))
                self.mod.connect(zc, "Y", ret_net, direction="output")
            results = self._collect_blocking_with_muxes(sub.body, allow_nb=False)
        finally:
            self._fn_scopes.pop()
        ret_val = results.get(f"{prefix}{ret_name}")
        if ret_val is None and ret_net is not None and ret_net.driver is not None:
            ret_val = ret_net  # body assigned nothing detectable; return the seed
        if ret_val is not None and ret_val.driver is not None:
            return ret_val
        return None
    ...
    def lower_instance(self, inst: Any) -> None:
        ...
        def walk_member(node: Any) -> None:
            ...
            if kind == "SymbolKind.Variable":
                ...
                if node.initializer is not None and net.driver is None:
                    ...
                    if rhs.driver is not None and net.driver is None:
                        ...
                        # `wire net = rhs` where rhs has no driver yet (it is
                        # driven by a sub-instance port or a later block). This
                        # is a continuous assign; resolve it after all blocks.
                        if not hasattr(self, '_deferred_net_inits'):
                            self._deferred_net_inits: list[tuple] = []
                        self._deferred_net_inits.append((net, rhs))
            ...
            # UDP (User-Defined Primitives) — reject with warning
            ...
            # Assertions, coverpoints, specify blocks — strip with warning
        ...
        # Resolve `wire x = y` aliases whose RHS is now driven (sub-instance
        # outputs and later blocks are wired by this point).
        self._resolve_deferred_net_inits()
    ...
    def _resolve_deferred_net_inits(self) -> None:
        """Resolve `wire net = rhs` aliases whose RHS was undriven at decl time.

        By now the RHS (a sub-instance output port or a later block) has a
        driver. Alias `net` to `rhs` by pointing net.driver at rhs's driver
        and redirecting every consumer of net to rhs, so the net is no longer
        undriven and reads the aliased value.
        """
        for net, rhs in getattr(self, '_deferred_net_inits', []):
            if net.driver is not None or rhs.driver is None:
                continue
            for cell in self.mod.cells.values():
                for pn, pnet in list(cell.inputs.items()):
                    if pnet is net:
    ...
    def _lower_sub_instance(self, inst: Any) -> None:
            ...
            if kind == "SymbolKind.Variable":
                ...
                if node.initializer is not None and net.driver is None:
                    ...
                    if rhs.driver is not None and net.driver is None:
                        ...
                        if not hasattr(sub, '_deferred_net_inits'):
                            sub._deferred_net_inits = []
                        sub._deferred_net_inits.append((net, rhs))
            elif kind == "SymbolKind.Parameter":
                w = sub._bit_width(node)
                if node.value is not None:
                    int_val = _svint_to_int(node.value)
        ...
        # Recover connection targets from the instantiation syntax. slang
        # leaves a connection's elaborated expression `Invalid` when the
        # connected net is forward-referenced (used in the port list above
        # its own declaration). The textual `.port(net)` survives in the
        # syntax tree, so map port name -> simple-identifier connection text.
        _syntax_conn: dict[str, str] = {}
        _isyn = getattr(inst, "syntax", None)
        for _c in getattr(_isyn, "connections", []) or []:
            if str(getattr(_c, "kind", "")) == "SyntaxKind.NamedPortConnection":
                _cexpr = getattr(_c, "expr", None)
                if _cexpr is not None:
                    _txt = str(_cexpr).strip()
                    if _re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", _txt):
                        _syntax_conn[str(_c.name).strip()] = _txt
        ...
        for conn in inst.portConnections:
            ...
            # Forward-referenced connections elaborate to Invalid; resolve the
            # parent net from the recovered syntax text instead of a bogus
            # unsupported-expression placeholder.
            if str(getattr(expr, "kind", "")) == "ExpressionKind.Invalid" and port_name in _syntax_conn:
                parent_net = self._get_or_create_net(_syntax_conn[port_name], w)
            else:
                parent_net = self.lower_expr(expr)
        ...
        # Resolve `wire x = y` aliases inside the sub-instance now that its
        # ports are wired (the RHS may be an input port or a later block).
        sub._resolve_deferred_net_inits()
