...
from nosis.passes.mux import merge_mux_chains
from nosis.passes.wreduce import reduce_register_width
...
def _repair_ff_output_aliases(mod: Module) -> int:
    """Redirect readers of stale driver aliases to the driver's true output.

    Lowering-time renaming and const-FF replacement can leave a net whose
    ``.driver`` points at a cell that does not actually list it among its
    outputs (the cell outputs to a different net; consumers were redirected
    piecemeal). Readers of the stale net map to netlist bits nothing drives —
    silently frozen or floating logic at gate level. Width must match: a
    width-reduced register's original net legitimately differs and its
    readers were already moved to the reconstruction.
    """
    # Collect every net object actually referenced — cell inputs and module
    # ports included. A stale alias can be orphaned out of mod.nets entirely
    # while cells still hold the object.
    candidates: dict[int, Net] = {}
    for net in mod.nets.values():
        candidates[id(net)] = net
    for cell in mod.cells.values():
        for pnet in cell.inputs.values():
            candidates[id(pnet)] = pnet
    for pnet in mod.ports.values():
        candidates[id(pnet)] = pnet

    # ff_target name -> true Q net, for the driverless-alias fallback.
    tgt_to_q: dict[str, Net] = {}
    for cell in mod.cells.values():
        if cell.op == PrimOp.FF:
            tgt = str(cell.params.get("ff_target", ""))
            q = next(iter(cell.outputs.values()), None)
            if tgt and q is not None:
                tgt_to_q[tgt] = q
                tgt_to_q.setdefault(q.name, q)

    stale: dict[int, Net] = {}
    for net in candidates.values():
        drv = net.driver
        if drv is not None:
            if any(o is net for o in drv.outputs.values()):
                continue
            true_out = next(iter(drv.outputs.values()), None)
            if (true_out is not None and true_out is not net
                    and true_out.width == net.width):
                stale[id(net)] = true_out
                net.driver = None
        else:
            # Driverless alias of a register: the name matches an FF target
            # (or its Q) whose actual Q is a different net object.
            q = tgt_to_q.get(net.name)
            if q is not None and q is not net and q.width == net.width:
                stale[id(net)] = q
    ...
    for cell in mod.cells.values():
        for pn, pnet in list(cell.inputs.items()):
            if id(pnet) in stale:
                cell.inputs[pn] = stale[id(pnet)]
                fixed += 1
    for pn, pnet in list(mod.ports.items()):
        if id(pnet) in stale:
            mod.ports[pn] = stale[id(pnet)]
            fixed += 1
    return fixed
...
def run_default_passes(mod: Module, *, verify: bool = False) -> dict[str, int]:
    ...
    # Collapse constant-case mux chains (S-boxes, sine tables) into ROM
    # cells before the iterative optimizer grinds on thousands of
    # EQ/MUX links per lookup.
    from nosis.passes.romextract import extract_roms
    stats["rom_extract"] = extract_roms(mod)
    if stats["rom_extract"]:
    ...
    for iteration in range(_max_iters):
        ...
        # Run identity simplification to a local fixed point
        ident = 0
        for _ in range(10):
        ...
        total = cf + ident + wr + bo + cff + cse + fi + hit + dci + mm + ta + dce
    ...
    # Repair FF output aliasing before anything builds on the net graph:
    # lowering can leave a net whose .driver points at an FF while the FF's
    # actual Q is a different ($ff_q_*) net, with only some consumers
    # redirected. Anything still reading the stale net maps to bits nothing
    # drives — silently frozen logic at gate level.
    stats["ff_alias_repair"] = _repair_ff_output_aliases(mod)

    # Native clock-enable extraction: turn hold-mux registers into FF+CE and
    # drop the freed feedback muxes.
    from nosis.passes.clockenable import extract_clock_enables
    stats["clock_enable"] = extract_clock_enables(mod)
    if stats["clock_enable"]:
        dead_code_eliminate(mod)
    ...
    # SAT-proven equivalence merging.
    ...
    # Annotate EQ comparisons for carry chain mapping
