...
class _ECP5Mapper:
    ...
    def _map_ff(self, cell: Cell) -> None:
        ...
        if cell.name in getattr(self, '_ce_candidates', {}):
            ...
            ce_net = ce_sel
            actual_d = ce_data
        elif cell.inputs.get("CE") is not None:
            # Active-high clock enable already extracted at the IR level
            # (extract_clock_enables). D is the update value; the FF holds Q
            # when CE is low, so no hold-MUX remains to strip.
            ce_net = cell.inputs["CE"]
    ...
    def _map_rom(self, cell: Cell) -> None:
        """Map a ROM (constant lookup table) to balanced LUT4 trees.

        Per output bit: one leaf LUT4 per low-selector nibble holding 16
        table entries, then a balanced 2:1 mux tree over the high selector
        bits. An 8-bit selector costs 16 leaves + 15 muxes per output bit,
        versus the ~256-cell chain the generic path would emit.
        """
        a_net = cell.inputs.get("A")
        out_nets = list(cell.outputs.values())
        if a_net is None or not out_nets:
            return
        out_net = out_nets[0]
        values = [int(v) for v in cell.params.get("values", [])]
        sel_bits = self._get_bits(a_net)
        w = min(a_net.width, len(sel_bits))
        depth = 1 << w
        if len(values) < depth:
            values = values + [0] * (depth - len(values))
        out_bits = self._get_bits(out_net)

        # INIT for Z = C ? B : A (pin D unused), replicated over D.
        mux_init = 0
        for idx in range(16):
            pa, pb, pc = idx & 1, (idx >> 1) & 1, (idx >> 2) & 1
            if (pb if pc else pa):
                mux_init |= 1 << idx
        mux_init_str = format(mux_init, "016b")

        low = sel_bits[:min(4, w)]
        high = sel_bits[4:w]

        for bit_i in range(out_net.width):
            if bit_i >= len(out_bits):
                break
            out_bit = out_bits[bit_i]
            if not isinstance(out_bit, int):
                continue  # already tied constant

            def _leaf_init(h: int) -> int:
                init = 0
                for li in range(16):
                    entry = (h << len(low)) | (li & ((1 << len(low)) - 1))
                    if entry < depth and (values[entry] >> bit_i) & 1:
                        init |= 1 << li
                return init

            def _emit_lut(init_str: str, pins: list, z_bit: int) -> None:
                lut = self.nl.add_cell(self._fresh_name("romlut"), "LUT4")
                if cell.src:
                    lut.attributes["src"] = cell.src
                lut.parameters["INIT"] = init_str
                for pi, pn in enumerate(("A", "B", "C", "D")):
                    lut.ports[pn] = [pins[pi]] if pi < len(pins) else ["0"]
                lut.ports["Z"] = [z_bit]

            n_leaves = 1 << len(high)
            if n_leaves == 1:
                # Selector fits one LUT: the leaf drives the output bit.
                _emit_lut(format(_leaf_init(0), "016b"), list(low), out_bit)
                continue

            # Leaf LUTs over the low selector bits (constant-INIT leaves are
            # emitted too; constant propagation folds them downstream).
            level: list[int] = []
            for h in range(n_leaves):
                zb = self.nl.alloc_bit()
                _emit_lut(format(_leaf_init(h), "016b"), list(low), zb)
                level.append(zb)
            # Balanced mux tree over the high selector bits; the final root
            # drives the ROM's own output bit.
            for hj, hbit in enumerate(high):
                last_level = (hj == len(high) - 1)
                nxt: list[int] = []
                for i in range(0, len(level), 2):
                    a_leaf = level[i]
                    b_leaf = level[i + 1] if i + 1 < len(level) else level[i]
                    zb = out_bit if last_level and i == 0 else self.nl.alloc_bit()
                    _emit_lut(mux_init_str, [a_leaf, b_leaf, hbit], zb)
                    nxt.append(zb)
                level = nxt
    ...
    def _map_arithmetic(self, cell: Cell) -> None:
        ...
        #
        # The hardware carry (matching the validated CCU2C_SIM model) is
        # computed from pins A and B of each half:
        #   S    = INIT[{D,C,B,A}] XOR CIN
        #   COUT = (A & B) | (CIN & (A ^ B))
        # so both operands MUST ride the A/B pins: A = a bit, B = b bit,
        # C = D = 0, INIT = A XOR B. Constant operand bits ride the pins as
        # "0"/"1" strings, which the carry expression honors identically.
        #
        # SUB computes a - b as a + ~b + 1: the b bits are inverted (constant
        # bits flipped in place, variable bits through a NOT LUT) and the
        # chain starts with CIN = 1.
        is_sub = (cell.op == PrimOp.SUB)

        # INIT = A XOR B, replicated over the unused C/D pins.
        ...
        def _b_bit(i: int):
            ...
            if isinstance(bb, str):
                return "1" if bb == "0" else "0"
            zb = self.nl.alloc_bit()
            inv = self.nl.add_cell(self._fresh_name("subinv"), "LUT4")
            if cell.src:
                inv.attributes["src"] = cell.src
            inv.parameters["INIT"] = "0101010101010101"  # NOT A
            inv.ports["A"] = [bb]
            inv.ports["B"] = ["0"]
            inv.ports["C"] = ["0"]
            inv.ports["D"] = ["0"]
        ...
        prev_cout: int | str = "1" if is_sub else "0"
        ...
        for i in range(0, width, 2):
            ...
            if i + 1 < width:
                ...
                # Unused second half: zero operands so the carry passes
                # through unchanged only when harmless (S1 unconsumed).
                a1, b1 = "0", ("1" if is_sub else "0")
            ccu2c.ports["A1"] = [a1]
            ccu2c.ports["B1"] = [b1]
            ccu2c.ports["C1"] = ["0"]
            ccu2c.ports["D1"] = ["0"]
    ...
    def _map_memory(self, cell: Cell) -> None:
        ...
        if bram_config == "DP16KD_TILED":
            ...
            # Pick the LAST RADDR for port A. The instruction fetch continuous
            # assign is processed last during lowering (deferred assigns run
            # after procedural blocks), so its RADDR is the last one created.
            # This ensures the instruction fetch has priority on port A.
            raddr_net = cell.inputs.get("RADDR")
            if raddr_net is None:
                for _rk in sorted(cell.inputs, reverse=True):
                    if _rk.startswith("RADDR"):
                        raddr_net = cell.inputs[_rk]
                        break
            waddr_net = cell.inputs.get("WADDR")
            ...
            for drow in range(tiles_deep):
                ...
                for wcol in range(tiles_wide):
                    ...
                    for i in range(14):
                        ...
                        bit = raddr_bits[logical] if 0 <= logical < len(raddr_bits) else "0"
                        bram.ports[f"ADA{i}"] = [bit]
                    # Port B address: write address only
                    for i in range(14):
                        logical = i - _tile_addr_offset
                        bit = waddr_bits[logical] if 0 <= logical < len(waddr_bits) else "0"
                        bram.ports[f"ADB{i}"] = [bit]
            ...
            # Wire extra RDATA outputs to same DOA bits via buffer LUTs.
        ...
        # Fold a whole-array synchronous reset into the FF's built-in LSR:
        # the unrolled `if (rst) for i: mem[i] <= 0` loop lowers to an all-zero
        # constant-address write of every element under one enable. Only fold a
        # write-enable that zero-writes *every* element (an unambiguous
        # array-wide reset). Restricting to full coverage keeps the transform
        # safe: a partial zero-write could be a normal store that a later write
        # must override, but the original mapping applies const writes at lower
        # priority than variable writes while LSR is highest priority, so
        # promoting a non-reset zero-write would invert that order. A full reset
        # is mutually exclusive with the guarded stores, so priority is moot.
        # Dropping the per-bit reset MUX saves one LUT per bit per element (each
        # has a distinct hold input, so CSE cannot merge them).
        from collections import defaultdict as _dd
        _zero_we_entries: dict[int | str, set] = _dd(set)
        for _w, _writes in const_writes.items():
            for _wd, _we in _writes:
                if all(isinstance(b, str) and b == "0" for b in _wd):
                    _wb = _we[0] if _we else "0"
                    if _wb != "0":
                        _zero_we_entries[_wb].add(_w)
        reset_wes_global = {we for we, ent in _zero_we_entries.items() if len(ent) >= depth}
        reset_we_of: dict[int, int | str] = {}
        if reset_wes_global:
            for w_idx in list(const_writes):
                kept = [(wd, we) for (wd, we) in const_writes[w_idx]
                        if (we[0] if we else "0") not in reset_wes_global]
                const_writes[w_idx] = kept
            ordered = sorted(reset_wes_global, key=lambda x: str(x))
            lsr = ordered[0]
            for extra in ordered[1:]:
                o = self.nl.alloc_bit()
                lut = self.nl.add_cell(self._fresh_name("mrstor"), "LUT4")
                lut.parameters["INIT"] = "1110111011101110"  # A OR B
                lut.ports["A"] = [lsr]
                lut.ports["B"] = [extra]
                lut.ports["C"] = ["0"]
                lut.ports["D"] = ["0"]
        ...
        for w_idx in range(depth):
            ...
            for b_idx in range(width):
                ...
                # Apply constant-address writes targeting this element. Emit a
                # plain 2:1 MUX per write and let the post-map LUT-merge passes
                # pack them — pre-packing constant data into 4-input LUTs here
                # measured worse, because a full LUT leaves no free input for
                # those passes to absorb an adjacent cell into.
                ...
                ff.ports["LSR"] = [reset_we_of.get(w_idx, "0")]
                ff.ports["Q"] = [q_bit]
                q_bits.append(q_bit)
        ...
        for rdata_net in cell.outputs.values():
            ...
            for b_idx in range(width):
                ...
                for a_i in range(len(raddr_bits)):
                    ...
                    # A constant select bit (a `mem[const]` field read) picks a
                    # half outright — collapse it to a wire instead of a MUX so
                    # a constant-address read costs no LUTs at all.
                    sel_const = sel if isinstance(sel, str) and sel in ("0", "1") else None
...
def map_to_ecp5(design: Design) -> ECP5Netlist:
    ...
    # Late optimization passes can leave stale FF-output aliases (a net whose
    # .driver is an FF that actually outputs to a different net); readers of
    # the stale net would map to undriven bits. Repair unconditionally before
    # mapping.
    from nosis.passes.pipeline import _repair_ff_output_aliases
    _repair_ff_output_aliases(mod)
