...
def _net_key(net) -> str:
    """Canonical name for a signature input: constants unify by value.

    Every lowered branch mints its own CONST cell for a literal, so `x + 1`
    in ten case arms produces ten adders whose B operands are ten different
    nets all driving the value 1. Keying constants by (value, width) instead
    of net name lets those adders deduplicate.
    """
    dr = net.driver
    if dr is not None and dr.op == PrimOp.CONST and not dr.params.get("_dead"):
        return f"$=const:{int(dr.params.get('value', 0))}:{net.width}"
    return net.name
...
def _cell_signature(cell: Cell) -> tuple | None:
    ...
    if cell.op not in _CSE_OPS:
        return None
    input_key = tuple(sorted((port, _net_key(net)) for port, net in cell.inputs.items()))
    try:
