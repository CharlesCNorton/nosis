...
Compiles a module into a flat instruction list once, then evaluates it
per step without dict lookups, topological sorting, or dispatch overhead.
The per-op functions intentionally mirror the semantics in ``nosis.eval``
(the reference evaluator) as a flattened hot path; any semantic change
...
class FastSimulator:
    ...
    def __init__(self, mod: Module) -> None:
        ...
        for cell in order:
            ...
            # ROM — special: constant lookup table
            if cell.op == PrimOp.ROM:
                a_net = cell.inputs.get("A")
                a_idx = net_index.get(a_net.name, -1) if a_net else -1
                values = tuple(int(v) for v in cell.params.get("values", ()))
                self._all_instructions.append(('rom', out_idx, a_idx, values, mask))
                continue
    ...
    def step(self, input_values: dict[str, int]) -> dict[str, int]:
        ...
        for instr in self._all_instructions:
            ...
            if kind == 'op':
                ...
                _, out_idx, a_idx, values, mask = instr
                a = vals[a_idx] if a_idx >= 0 else 0
                vals[out_idx] = (values[a] if a < len(values) else 0) & mask
